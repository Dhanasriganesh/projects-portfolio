...
export default function ProcessLineReveal() {
  ...
  return (
    <section ref={root} className="relative bg-white py-20 sm:py-28">
      <div className="pointer-events-none absolute right-0 top-0 h-[min(55vh,480px)] w-[min(55vw,420px)] translate-x-1/4 rounded-full bg-gradient-to-bl from-indigo-200/50 via-transparent to-transparent blur-3xl" aria-hidden />
      <Container>
        <div data-process-title className="max-w-3xl will-change-transform">
          <p className="text-xs font-extrabold uppercase tracking-[0.35em] text-ink-muted">Process</p>
        ...
        <div className="relative mt-16 grid gap-10 lg:grid-cols-12">
          <div className="relative hidden justify-center self-stretch lg:col-span-1 lg:flex">
            <div className="relative h-[min(520px,72vh)] w-3 shrink-0 py-4">
              <div className="absolute inset-y-0 left-1/2 w-1 -translate-x-1/2 rounded-full bg-indigo-100" aria-hidden />
              <div
                data-process-fill
                className="absolute inset-y-0 left-1/2 w-[5px] -translate-x-1/2 rounded-full bg-gradient-to-b from-indigo-300 via-indigo-500 to-indigo-700 will-change-transform"
          ...
          <ol className="grid gap-6 lg:col-span-11 lg:grid-cols-2">
            {process.map((p) => (
              <li
                key={p.step}
                data-process-node
                className="relative overflow-hidden rounded-[1.75rem] border border-brand bg-accent-soft p-8 pl-10 shadow-md ring-1 ring-indigo-200 will-change-transform lg:pl-8"
              >
                <span
                  data-process-dot
                ...
                <div className="text-[11px] font-extrabold uppercase tracking-[0.35em] text-ink-muted">{p.step}</div>
                <div className="mt-2 font-display text-xl font-extrabold tracking-tight text-ink">{p.title}</div>
                <p className="mt-4 text-sm leading-relaxed text-ink-muted sm:text-base">{p.detail}</p>
              </li>
            ))}
          </ol>
        </div>
